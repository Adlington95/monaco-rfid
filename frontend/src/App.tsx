            ...
            {monacoData.map((row: any, index: number) => (
              <tr key={index} style={{padding: '12px'}}>
                {COLUMNS.map((column: any, colIndex: number) => (
                  column === 'Name' ? <td style={{padding: '12px'}} key={colIndex}>{row.name}</td> :
                  column === 'Team Name' ? <td style={{padding: '12px'}} key={colIndex}>{row.team_name}</td> :
                  <td style={{padding: '12px'}} key={colIndex}>{row.lap_time}</td>
                ))}
              </tr>
            ))}
          </tbody>
